...
public class ParticleEnv {
    ...
    public ParticleEnv(){
        ...
        for(int i = 0; i < 1500; i++) {
            globalParticles.add(new Particle((int)(Math.random() * maxY), (int)(Math.random() * maxX)));
        }
        ...
        for(int i = 0; i < 9; i++) {
            for (Particle n : getNearbyParticles(i, p)) {
                ...
                if(p != n) {
                    ...
                    xForce = 2*force * Math.cos(Math.atan(divide((p.y-n.y),(p.x-n.x))));
                    yForce = 2*force * Math.cos(Math.atan(divide((p.x-n.x), (p.y-n.y))));
    ...
    private void calculateForceOfNearbyWalls(Particle p) {
        ...
        if(p.y > maxY - 5) {p.yForce = -12;};      //bottom wall
    }
    ...
    private List<Particle> getNearbyParticles(int i, Particle p) {
        ...
        else if(i == 4) {return localParticles[p.localIndexY][p.localIndexX];}                                                                                    //left neighbour
        ...
        else if(i == 7 && p.localIndexY < Math.ceil(maxY/40)) {return localParticles[p.localIndexY+1][p.localIndexX];}                                            //bottom left neighbour
        else if(i == 8 && p.localIndexX > Math.ceil(maxX/40) && p.localIndexY < Math.ceil(maxY/40)) {return localParticles[p.localIndexY+1][p.localIndexX+1];}    //bottom left neighbour
